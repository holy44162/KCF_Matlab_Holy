...
function [precision, fps, positions, distances] = demo_run_tracker(video, kernel_type, feature_type, show_visualization, show_plots)

	%path to the videos (you'll be able to choose one with the GUI).
% 	base_path = './data/Benchmark/'; % hided by Holy 1903102010
    base_path = 'd:/data_seq/changdeWinding/miniDataSets/'; % added by Holy 1903102010
	...
	case 'choose'
		...
		if ~isempty(video)
			[precision, fps, positions, distances] = demo_run_tracker(video, kernel_type, ...
...
% 			end
	...
	otherwise
		...
		[img_files, pos, target_sz, ground_truth, video_path] = load_video_info_holy(base_path, video);
		
		
		%call tracker function with all the relevant parameters
		[positions, time] = tracker_Holy(video_path, img_files, pos, target_sz, ...
			padding, kernel, lambda, output_sigma_factor, interp_factor, ...
			cell_size, features, show_visualization);
		
		
		%calculate and show precision plot, as well as frames-per-second
		[precisions,distances] = precision_plot(positions, ground_truth, video, show_plots);
